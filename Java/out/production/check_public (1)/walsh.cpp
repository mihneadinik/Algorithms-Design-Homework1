...
int find_pos(int x, int y, int n, int nr_neg) {
    ...
    if (x <= ((n >> 1) - 1) && y > ((n >> 1) - 1))  // C2
        return find_pos(x, y - (n >> 1), n >> 1, nr_neg);  // shift the indexes to fit the new matrix size
...
void solve(int n, int w, std::vector<std::pair<int, int>> &queries) {
    std::ofstream out;
    out.open(file_out);

    for (int i = 0; i < w; i++) {
        // find the max dim
        int maxi = queries[i].first;
        if (maxi < queries[i].second)
            maxi = queries[i].second;

        // find the matrix size (multiply by 2 while the max dim is bigger than the matrix border)
        int matrix_size = 0;
        while (maxi > (1 << matrix_size)) {
            matrix_size++;
        }

        int res = find_pos(queries[i].first - 1, queries[i].second - 1, (1 << matrix_size), 0);
        out << res << std::endl;
    }
